...
traders-own [ppf plan endowment old-endowment last-trade strategies]
;trades-own [mix1 mix2 prob]

to setup
  clear-all
  create-traders num-agents [
    setxy random-xcor random-ycor
    set ppf n-values num-goods [1 + random num-agents]
    set plan n-values num-goods [1]
    set endowment n-values num-goods [10]
    set old-endowment endowment
    set strategies n-values num-strategies [
      (list random-mix random-mix 1)
    ]
  ]
  reset-ticks
end
...
to setup-simple
  ...
  set num-goods 2
  setup
  ask turtle 0 [set ppf [1 2]]
  ask turtle 1 [set ppf [2 1]]
end
...
to produce
  set endowment (
    map [[x y z]-> x + y * z] endowment ppf softmax plan
  )
end

; treat trades as a breed of turtle
...
;  let trade rnd:weighted-one-of-list strategies [[p] -> last p]
  ...
  if my-gain and their-gain [
    ...
    set strategies replace-item trade-index strategies trade; update probability based on success
    ask partner [undertake-trade reverse-trade trade]
  ]
end

to-report reverse-trade [trade]
  let give first butfirst trade
  let take first trade
  report (list give take last trade)
end

;; boolean reporter to evaluate a trade
...
to-report partner-evaluate-trade [ partner trade ]
  report [evaluate-trade reverse-trade trade] of partner
end


to undertake-trade [ trade ]
  let give first trade
  let take first butfirst trade
...
to-report all-gt [vect1 vect2]
  ; are all elements of vect1 strictly greater than vect2?
  report reduce and (map [[v1 v2]-> v1 > v2] vect1 vect2)
end

to-report all-gte [vect1 vect2]
  ; are all elements of vect1 greater than or equal to vect2?
  report reduce and (map [[v1 v2]-> v1 >= v2] vect1 vect2)
end

to-report softmax [vect]
  let S sum vect
  report (map [[v] -> v / S ]vect)
end

to-report diff [vect1 vect2]
  report (map [[v1 v2] -> v1 - v2] vect1 vect2)
end
@#$#@#$#@
GRAPHICS-WINDOW
210
